...
```mermaid
graph TD
    A[User Browser (from Part 1)] -->|1. Direct Image Upload (via JS/Pre-signed URL)| B(Amazon S3 - Original Images Bucket)
    B -- Event Notification --> C(AWS Lambda - Image Processor Function)
    C -->|2. Process Image & Save| D(Amazon S3 - Processed Images Bucket)
    D -->|3. Serve Processed Image to Frontend| A
```
